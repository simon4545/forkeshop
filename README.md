...
##开发计划（RoadMap）：
物流快递查询：可以查询订单的快递详细信息及收货状态
促销活动：满减功能，满赠功能，秒杀功能，一元购功能
二级分销：二级分销功能，可结算，可管理分销商
售后功能：用户可对异常订单（退货，换货）进行处理，上传照片及描述，可以申请退换货，并在后台处理完成后，自动退款或退货
结算功能：以月或周为指定周期，统计商城销售成本及利润
优惠券（积分券）功能：可以设定商品的积分规则，在用户下单后返还积分，同时积分可以抵扣部分购物现金
多端：开发PC端及安卓,IOS端（目前只支持移动网页端）
