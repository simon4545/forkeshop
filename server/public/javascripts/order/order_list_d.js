...
layer.alert('导出发货中订单按钮已经 独立出来了，请注意页面中绿色的“<span style="color: green;font-weight: bolder"">导出发货中订单</span>”按钮哦~,红色的“<span style="color: red;font-weight: bolder">导出其他订单</span>”按钮用于导出对账订单使用哦!');
...
function exportOrders() {
    if(orderState == 2)  {
        layer.alert('待配货的订单无法导出~');
        return;
    }
    startDay = $('#startDay').val();
    endDay = $('#endDay').val();
    if(startDay && endDay || orderState == 3) {
        var udl = '/order/exportorders?orderstate=' + orderState + '&startday='+startDay+'&endday='+endDay;
...
function bindCommands() {
    $('#btnExportOrder').on('click', function ()  {
        ...
        arr_html.push('<div style="float: right;" class="btn-group" onclick="return exportOrders(\'all\')"><button class="btn btn-info green">'+_btnHtml+'</button>');
...
oi.change = {
    ...
    getMerchantInfo: function (goodsId) {
        var url = '/order/getMerchant';
        var param = {goodsId: goodsId};
        common.ajax(url, param, oi.change._getMerchantSuc, oi.change._error);
    },
