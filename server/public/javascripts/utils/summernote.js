...
(function (factory) {
    ...
    var Editor = function (handler) {
         ...
         * @param {String} sUrl
         */
        this.insertImage = function ($editable, sUrl, filename, next) {
            async.createImage(sUrl, filename).then(function ($image) {
                ...
                //var span = $("<span><br/></span>").append($image);
                range.create().insertNode($image[0]);
                range.createFromNodeAfter($image[0]).select();
                //range.create().insertNode(span[0]);
                //range.createFromNodeAfter(span[0]).select();
                afterCommand($editable);
                if (next) {
                    console.log(sUrl, new Date().getTime());
                    next();
                }
            }).fail(function () {
                var $holder = dom.makeLayoutInfo($editable).holder();
            ...
            this.showImageDialog($editable, $dialog).then(function (data) {
                ...
                if (typeof data === 'string') {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var func = function () {
                            var url = arguments.callee.url;
                            handler.insertSingleImage(layoutInfo, url, insert);
                        }
                        func.url = url;
                        funcArr.push(func);
                    }
                    ...
                    $(document).queue("insertImage", funcArr);
                    insert();
                }
            }).fail(function () {
                handler.invoke('editor.restoreRange', $editable);
        ...
        this.showImageDialog = function ($editable, $dialog) {
            return $.Deferred(function (deferred) {
                ...
                $imageDialog.one('shown.bs.modal', function () {
                    ...
                    $imageInput.replaceWith($imageInput.clone().val('')
                            .on('change', function () {
                                var allSmallThan700kb = true;
                                var filesObj = $(this)[0].files;
                                for (var attr in filesObj) {
                                    var size = filesObj[attr].size;
                                    if (size > 720000) {
                                        allSmallThan700kb = false;
                                    }
                                }
                                if (!allSmallThan700kb) {
                                    layer.alert("上传的图片中，有图片大于700KB，请修改后重新上传。");
                                    return;
                                }
                                //这里往云端传图片后获取到地址  ----luxing 2015-07-27
                                $(".loadingImgDiv").show();
                                //TODO 需要修改

                                 uploadFile('formUploadDetailImg', function (fileUrl) {
                                     deferred.resolve(fileUrl.msg);
                                     $(".loadingImgDiv").hide();
    ...
    var EventHandler = function () {
         ...
         */
        this.
            insertImages = function (layoutInfo, files) {
            var $editor = layoutInfo.editor(),
                $editable = layoutInfo.editable(),
        ...
        this.insertSingleImage = function (layoutInfo, sDataURL, next) {
            var _self = this;
            var $editable = layoutInfo.editable();
            _self.modules.editor.insertImage($editable, sDataURL, "", next);
        }
    ...
    var Renderer = function () {
        ...
        var tplDialogInfo = {
            image: function (lang, options) {
                ...
                var body = '<form class = "form-horizontal" enctype="multipart/form-data" method="post" action="javascript:;" role = "form" id = "formUploadDetailImg">' +
                    '<div class="form-group row-fluid note-group-select-from-files">' +
                    '<label>' + lang.image.selectFromFiles + '</label>' +
                    '<input id="note-image-input" class="note-image-input" type="file" name="files" accept="image/*" multiple="multiple"/>' + imageLimitation +
                    '</div>' +
                    '<div class="loadingImgDiv" style="display: none">' +
